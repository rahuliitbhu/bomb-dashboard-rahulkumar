...
const Bond: React.FC = () => {
  ...
  const {onReward} = useHarvest(banks[4]);
  const [approveStatus, approve] = useApprove(bombFinance.BSHARE, bombFinance.contracts.Boardroom.address);


  const {onStake} = useStake(banks[4]);
  const {onWithdraw} = useWithdraw(banks[4]);
  const canWithdrawFromBoardroom = useWithdrawCheck();

  const tokenBalance = useTokenBalance(banks[4].depositToken);
  ...
  const [onPresentDeposit, onDismissDeposit] = useModal(
    <DepositModal
      ...
      tokenName={'BSHARE-BNB-LP'} decimals={0}    />,
  );
      ...
      <Page>
        ...
        <title>{TITLE}</title>
          ...
          {/* <Card > */}
            ...
            <Div4>
            <TokenSymbol symbol="BSHARE-BNB-LP" size={40} />
           ...
           <div>
             <Typography>Your Stake:</Typography> 
              <Typography><TokenSymbol symbol="BSHARE-BNB-LP" size={20} />{getDisplayBalance(stakedBalance)} </Typography>
